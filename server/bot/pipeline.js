...
export function runPipeline(rawInputs, options = {}) {
  ...
  // ---- STAGE 1: SCOUT ----
  auditLog.push({ stage: 'scout', timestamp: new Date().toISOString(), status: 'running' });
  ...
  // ---- STAGE 2: CLUSTER ----
  auditLog.push({ stage: 'clusterer', timestamp: new Date().toISOString(), status: 'running' });
  const clusters = clusterSignals(scoutResult.candidates, options.clusteringOptions);
  auditLog.push({
    stage: 'clusterer',
    timestamp: new Date().toISOString(),
    status: 'complete',
  ...
  for (const cluster of clusters) {
    ...
    const counterCase = generateCounterCase(cluster, patternMatch, mispricingAssessment);
  ...
  const statePriority = { ESCALATE: 0, DEVELOP_THESIS: 1, QUARANTINE: 2, WATCH: 3, LOG_ONLY: 4, IGNORE: 5 };
  analyses.sort((a, b) =>
  ...
  const result = {
    ...
    summary: {
      ...
      escalations: analyses.filter(a => a.recommendation.recommended_state === 'ESCALATE').length,
      develop_thesis: analyses.filter(a => a.recommendation.recommended_state === 'DEVELOP_THESIS').length,
      quarantined: analyses.filter(a => a.recommendation.recommended_state === 'QUARANTINE').length,
      watch: analyses.filter(a => a.recommendation.recommended_state === 'WATCH').length,
      log_only: analyses.filter(a => a.recommendation.recommended_state === 'LOG_ONLY').length,
      ignored: analyses.filter(a => a.recommendation.recommended_state === 'IGNORE').length,
    },
...
export function runFullScan() {
  try {
    ...
    const rawInputs = [
      ...
      ...newsObs.map(o => ({ type: 'market_observation', data: parseJsonFields(o) })),
    ];
