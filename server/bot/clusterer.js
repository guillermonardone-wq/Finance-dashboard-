...
export function clusterSignals(candidates, options = {}) {
  ...
  for (let i = 0; i < sorted.length; i++) {
    ...
    for (let j = i + 1; j < sorted.length; j++) {
      ...
      const similarity = computeSimilarity(sorted[i], sorted[j], timeWindowHours);
...
function computeSimilarity(a, b, timeWindowHours) {
  ...
  maxScore += 0.15;
  const symbolOverlap = setOverlap(a.linked_market_symbols || [], b.linked_market_symbols || []);
...
function setOverlap(a, b) {
  ...
  const setA = new Set(a.map(x => (typeof x === 'string' ? x : JSON.stringify(x)).toLowerCase()));
  const setB = new Set(b.map(x => (typeof x === 'string' ? x : JSON.stringify(x)).toLowerCase()));
...
function buildCluster(members) {
  ...
  for (const m of members) {
    for (const ref of (m.source_refs || [])) {
      if (ref.attribution) sourceAttributions.add(ref.attribution.toLowerCase());
  ...
  const allGeographies = [...new Set(members.flatMap(m => m.geographies || []))];
  const allEntities = [...new Set(members.flatMap(m => m.entities || []))];
  const allSymbols = [...new Set(members.flatMap(m => m.linked_market_symbols || []))];

  // Determine cluster strength based on independence
  let strength = 'weak';
  if (independentSourceCount >= 4 && sourceTypes.size >= 3) strength = 'significant';
  else if (independentSourceCount >= 3 && sourceTypes.size >= 2) strength = 'strong';
  ...
  const duplicatePenalty = members.length > 2 && independentSourceCount < members.length * 0.4;
  ...
  const bestConfidence = members.reduce((max, m) => Math.max(max, m.confidence_range?.best || 0.3), 0);
  ...
  const summary = summaryParts.join(' | ') + (members.length > 3 ? ` (+${members.length - 3} more)` : '');
