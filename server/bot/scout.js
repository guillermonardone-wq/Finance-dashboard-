...
const DRAMATIC_PATTERNS = [
  /\bbreaking\b/i, /\bshocking\b/i, /\bunprecedented\b/i, /\bcrisis\b/i,
  /\bcollapse\b/i, /\bsurge\b/i, /\bplunge\b/i, /\bskyrocket\b/i,
  /\bexplode\b/i, /\bcrash\b/i, /\btumble\b/i, /\bsoar\b/i,
  /\bmassive\b/i, /\bhistoric\b/i, /\bbloodbath\b/i, /\bcatastroph/i,
  /\bdoomsday\b/i, /\bapocalyp/i,
];

// Category keyword mapping for auto-categorization
const CATEGORY_KEYWORDS = {
  geopolitical_escalation: ['geopolitical', 'escalation', 'conflict', 'tensions', 'confrontation', 'diplomacy', 'summit'],
  military_mobilization: ['military', 'troops', 'deployment', 'mobilization', 'navy', 'carrier', 'missile', 'strike', 'defense', 'army', 'forces'],
  commodity_chokepoint: ['chokepoint', 'strait', 'canal', 'pipeline', 'route', 'passage', 'hormuz', 'suez', 'malacca', 'bosphorus'],
  sanctions_risk: ['sanction', 'embargo', 'restriction', 'blacklist', 'ban', 'tariff', 'penalty'],
  shipping_disruption: ['shipping', 'tanker', 'vessel', 'maritime', 'port', 'freight', 'cargo', 'shipping lane'],
  energy_bottleneck: ['energy', 'oil', 'gas', 'lng', 'crude', 'opec', 'refinery', 'petroleum', 'barrel'],
  policy_shock: ['policy', 'regulation', 'legislation', 'executive order', 'reform', 'deregulation'],
  currency_instability: ['currency', 'forex', 'yen', 'yuan', 'euro', 'dollar', 'devaluation', 'depreciation', 'fx'],
  market_complacency: ['complacency', 'volatility low', 'implied vol', 'risk premium', 'overvalued'],
  central_bank_action: ['central bank', 'fed', 'ecb', 'boj', 'pboc', 'rate decision', 'interest rate', 'quantitative', 'tightening', 'easing', 'yield curve'],
  election_political: ['election', 'vote', 'referendum', 'parliament', 'congress', 'president', 'prime minister'],
  supply_chain: ['supply chain', 'shortage', 'inventory', 'backlog', 'logistics', 'semiconductor'],
  credit_stress: ['credit', 'default', 'spread', 'cds', 'downgrade', 'bankruptcy', 'distress', 'debt'],
  conflict_kinetic: ['attack', 'bombing', 'invasion', 'war', 'combat', 'drone', 'airstrike', 'casualt'],
};

// Geography extraction keywords
const GEOGRAPHY_KEYWORDS = {
  'Middle East': ['iran', 'israel', 'saudi', 'gulf', 'hormuz', 'yemen', 'houthi', 'iraq', 'syria', 'lebanon'],
  'East Asia': ['china', 'japan', 'taiwan', 'korea', 'boj', 'pboc', 'yen', 'yuan'],
  'Europe': ['eu', 'ecb', 'germany', 'france', 'uk', 'nato', 'ukraine', 'russia'],
  'North America': ['us', 'fed', 'canada', 'mexico', 'usmca'],
  'Southeast Asia': ['asean', 'indonesia', 'vietnam', 'philippines', 'malacca', 'singapore'],
...
export function scoutSignals(rawInputs) {
  ...
  for (const input of rawInputs) {
    ...
    if (seenTitles.has(normalizedTitle)) {
      ...
      existing.source_refs = [...new Set([...existing.source_refs, ...candidate.source_refs])];
      continue;
    }
    ...
    const dramaticCount = DRAMATIC_PATTERNS.filter(p => p.test(candidate.title + ' ' + candidate.summary)).length;
    if (dramaticCount >= 2) {
      penalties.push({
      ...
      candidate.novelty_score = Math.max(0, (candidate.novelty_score || 0.5) - dramaticCount * 0.05);
      penalties_applied++;
    }
...
function processInput(input) {
  ...
  switch (type) {
    case 'signal':
      ...
      candidate.source_refs = [{ type: data.source_type, attribution: data.source_attribution, url: data.source_url }];
      candidate.categories = data.category ? [data.category] : [];
      candidate.novelty_score = data.novelty === 'new' ? 0.8 : data.novelty === 'developing' ? 0.6 : 0.3;
    ...
    case 'news':
      ...
      candidate.detected_at = data.publishedAt || data.published_at || new Date().toISOString();
      candidate.source_refs = [{
        type: 'news_feed',
        attribution: data.source_attribution || data.source || data.source_name || 'Unknown',
        url: data.url,
      }];
    ...
    case 'market_observation':
      if (data.observation_type === 'news') {
        ...
        candidate.source_refs = [{ type: 'market_data', attribution: data.source_attribution || data.provider }];
      } else {
  ...
  const urgencyKeywords = {
    critical: ['breaking', 'imminent', 'emergency', 'immediate', 'attack underway'],
