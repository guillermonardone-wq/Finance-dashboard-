...
import { BOT_STATES, validateBotRecommendation } from "./types.js";
...
export const PENALTIES = [
  {
    ...
    description:
      "Only one source detected. Independent confirmation is missing.",
    check: (inputs) => {
  ...
  },
  {
    ...
    description:
      "High dramatic intensity without corresponding concrete evidence.",
    check: (inputs) => {
      ...
      return {
        ...
        severity: 0.15,
        reason:
          "More than half of signals flagged for dramatic language without concrete operational content.",
      };
    },
    effect: "warn",
    cap_at: null,
    ...
    check: (inputs) => {
      ...
      if (!mispricingAssessment)
        return {
          applies: true,
          severity: 0.2,
          reason: "No mispricing assessment available.",
        };
      return {
        applies:
          mispricingAssessment.stale_data_penalty_applied ||
          mispricingAssessment.market_reaction_state === "unknown",
        severity: 0.25,
  ...
  },
  {
    ...
    check: (inputs) => {
      return {
        ...
        severity: 0.2,
        reason:
          "No market mispricing assessment performed. Cannot distinguish important from tradable.",
      };
    },
    effect: "cap",
    cap_at: "WATCH",
  },
  {
    ...
    check: (inputs) => {
      return {
        applies:
          !inputs.counterCase ||
          !inputs.counterCase.strongest_opposing_case ||
          inputs.counterCase.strongest_opposing_case.length < 20,
        severity: 0.3,
        reason:
          "No substantive counter case generated. Proceeding without disconfirmation is confirmation bias in action.",
      };
    },
    effect: "cap",
    cap_at: "WATCH",
  },
  {
    ...
    description:
      "Pattern match has high overlap with known false positive patterns.",
    check: (inputs) => {
  ...
  },
  {
    ...
    check: (inputs) => {
      const missingConfirmations =
        inputs.patternMatch?.missing_confirmations?.length || 0;
      return {
  ...
  },
  {
    ...
    description:
      "High drama, thin evidence. The most dangerous combination for this user profile.",
    check: (inputs) => {
      ...
      const weakMispricing =
        !mispricingAssessment ||
        (mispricingAssessment.implied_mispricing_likelihood || 0) < 0.4;
      const strongCounter =
        counterCase && (counterCase.reasoning_quality_score || 0) >= 6;
      return {
        applies:
          highNovelty && weakEvidence && (weakMispricing || strongCounter),
        severity: 0.35,
        reason:
          "This looks exciting but evidence is thin and/or the counter case is strong. This is exactly the pattern where you historically over-commit.",
      };
    },
    effect: "cap",
    cap_at: "QUARANTINE",
  ...
  },
  {
    ...
    check: (inputs) => {
      ...
      const missing = signals.filter(
        (s) => !s.source_refs || s.source_refs.length === 0,
      );
      return {
...
export const ROUTING_RULES = [
  {
    ...
    description:
      "One source only AND no market confirmation AND no playbook match → LOG_ONLY or WATCH",
    check: (inputs) => {
      const singleSource = (inputs.cluster?.independent_source_count || 0) <= 1;
      const noMarket =
        !inputs.mispricingAssessment ||
        inputs.mispricingAssessment.market_reaction_state === "unknown";
      const noPattern =
        !inputs.patternMatch || (inputs.patternMatch.match_score || 0) < 0.3;
      return singleSource && noMarket && noPattern;
    },
    state: "LOG_ONLY",
    why_not_higher:
      "Single source with no market data and no pattern match. Insufficient basis for active monitoring.",
  },
  {
    id: "rule_B",
    name: "High cluster strength but duplicate-heavy and low mispricing",
    description:
      "Cluster strength high BUT duplicate-heavy AND mispricing low → WATCH",
    check: (inputs) => {
      const strongCluster = ["moderate", "strong", "significant"].includes(
        inputs.cluster?.cluster_strength,
      );
      const duplicateHeavy =
        (inputs.cluster?.independent_source_count || 0) <
        (inputs.cluster?.signal_count || 0) * 0.4;
      const lowMispricing =
        !inputs.mispricingAssessment ||
        (inputs.mispricingAssessment.implied_mispricing_likelihood || 0) < 0.3;
      return strongCluster && duplicateHeavy && lowMispricing;
    },
    state: "WATCH",
    why_not_higher:
      "Cluster appears strong but is inflated by duplicate coverage, and market is not obviously mispricing. Volume of coverage ≠ independent confirmation.",
  },
  {
    id: "rule_C",
    name: "Pattern match strong but critical confirmations missing",
    description:
      "Pattern match strong BUT critical confirmations missing → WATCH or QUARANTINE",
    check: (inputs) => {
      const strongPattern =
        inputs.patternMatch && (inputs.patternMatch.match_score || 0) >= 0.6;
      const missingCritical =
        (inputs.patternMatch?.missing_confirmations?.length || 0) >= 3;
      return strongPattern && missingCritical;
    },
    state: "WATCH",
    why_not_higher:
      "Pattern match is suggestive but too many expected confirmations are absent. The pattern is incomplete.",
  },
  {
    id: "rule_D",
    ...
    check: (inputs) => {
      ...
      const repriced =
        inputs.mispricingAssessment?.market_reaction_state === "fully_repriced";
      return highNovelty && repriced;
    },
    state: "LOG_ONLY",
    why_not_higher:
      "Event is notable but market has already repriced. Important ≠ mispriced. The opportunity window appears closed.",
  },
  {
    id: "rule_E",
    ...
    state: "QUARANTINE",
    why_not_higher:
      "This is exactly the trap: exciting, dramatic, but under-validated. Quarantined until evidence improves. Come back when you have independent confirmation.",
  },
  {
    id: "rule_F",
    name: "Multi-source, pattern match, plausible mispricing, red-team not dominant",
    description:
      "Independent sources ≥ 3 AND pattern match strong AND mispricing plausible AND red-team not dominant → DEVELOP_THESIS",
    check: (inputs) => {
      const multiSource = (inputs.cluster?.independent_source_count || 0) >= 3;
      const strongPattern =
        inputs.patternMatch && (inputs.patternMatch.match_score || 0) >= 0.5;
      const mispricingPlausible =
        inputs.mispricingAssessment &&
        (inputs.mispricingAssessment.implied_mispricing_likelihood || 0) >= 0.4;
      const redTeamNotDominant =
        !inputs.counterCase ||
        (inputs.counterCase.reasoning_quality_score || 0) < 7;
      return (
        multiSource &&
        strongPattern &&
        mispricingPlausible &&
    ...
    why_not_higher:
      "Evidence supports structured thesis development. Action states require user-side gating — the bot layer does not approve trades.",
  },
  {
    id: "rule_G",
    ...
    check: (inputs) => {
      ...
      const veryStale =
        (inputs.mispricingAssessment?.data_freshness_hours || 0) > 48;
      return stale || veryStale;
    },
    state: "WATCH",
    why_not_higher:
      "Supporting data is stale. Cannot make routing decisions on outdated market information.",
  },
];
...
export const ESCALATION_RULES = [
  {
    ...
    check: (inputs) => {
      ...
      const recentSignals = (inputs.candidateSignals || []).filter((s) => {
        const age =
          (Date.now() - new Date(s.detected_at).getTime()) / (1000 * 3600);
        return age < 1;
      });
      return recentSignals.length >= 3 && signalCount >= 5;
    },
    reason:
      "Cluster is growing rapidly — multiple new signals in the last hour.",
  },
  {
    id: "high_urgency_multi_source",
    name: "High Urgency + Multi-Source",
    check: (inputs) => {
      const urgentSignals = (inputs.candidateSignals || []).filter(
        (s) => s.urgency_score === "critical" || s.urgency_score === "high",
      );
      const multiSource = (inputs.cluster?.independent_source_count || 0) >= 2;
      return urgentSignals.length >= 2 && multiSource;
    },
    reason:
      "Multiple high-urgency signals from independent sources. May require immediate review.",
  },
  {
    ...
    check: (inputs) => {
      const strongPattern =
        inputs.patternMatch && (inputs.patternMatch.match_score || 0) >= 0.7;
      // Check if any signals mention imminent events
      const imminentSignals = (inputs.candidateSignals || []).filter(
        (s) => s.urgency_score === "critical",
      );
...
export function runGovernor(inputs) {
  ...
  let base_why_not_higher =
    "No routing rule matched. Defaulting to LOG_ONLY — insufficient signal quality.";
  ...
  if (inputs.counterCase) {
    ...
    if (
      counterQuality >= 7 &&
      BOT_STATES.indexOf(recommended_state) > BOT_STATES.indexOf("WATCH")
    ) {
      governor_overrides.push({
        from: recommended_state,
        to: "WATCH",
      ...
      downgrade_reasons.push(
        `Red-team dominant: counter case quality ${counterQuality}/10`,
      );
    }
  }
  ...
  if (inputs.patternMatch?.missing_confirmations) {
    required_next_confirmations.push(
      ...inputs.patternMatch.missing_confirmations.map((mc) =>
  ...
  if (recommended_state === "DEVELOP_THESIS") {
    required_user_actions.push(
      "Create structured thesis with full disconfirmation case",
    );
  ...
  if (governor_overrides.length > 0) {
    why_not_higher_parts.push(
      "Governor overrides applied: " +
        governor_overrides.map((o) => o.reason).join("; "),
    );
  }
  ...
  const clusterConf = inputs.cluster?.confidence_range || {
    low: 0.1,
    high: 0.5,
    best: 0.3,
  };
...
function buildRationale(inputs, state, matchedRule, penalties, overrides) {
  ...
  if (inputs.cluster) {
    parts.push(
      `Cluster: ${inputs.cluster.signal_count || 0} signals, ${inputs.cluster.independent_source_count || 0} independent sources, strength: ${inputs.cluster.cluster_strength}`,
    );
  }

  if (inputs.patternMatch) {
    parts.push(
      `Pattern: ${inputs.patternMatch.pattern_name} (match: ${((inputs.patternMatch.match_score || 0) * 100).toFixed(0)}%)`,
    );
  }

  if (inputs.mispricingAssessment) {
    parts.push(
      `Market: ${inputs.mispricingAssessment.market_reaction_state}, mispricing likelihood: ${((inputs.mispricingAssessment.implied_mispricing_likelihood || 0) * 100).toFixed(0)}%`,
    );
  }

  if (inputs.counterCase) {
    parts.push(
      `Counter case quality: ${inputs.counterCase.reasoning_quality_score || 0}/10`,
    );
  }

  if (penalties.length > 0) {
    parts.push(
      `Penalties: ${penalties.length} applied (${penalties.map((p) => p.id).join(", ")})`,
    );
  }
