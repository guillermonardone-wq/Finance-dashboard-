 ...
 */
export function generateCounterCase(cluster, patternMatch = null, mispricingAssessment = null) {
  ...
  const circular = (cluster.independent_source_count || 0) <= 1 &&
  ...
  if (mispricingAssessment?.market_reaction_state === 'fully_repriced') quality += 1;
  ...
  const validated = validateCounterCase({
    ...
    pricing_already_reflects_story: mispricingAssessment?.market_reaction_state === 'fully_repriced' ||
    ...
    notes: circular ? 'CIRCULAR LOGIC DETECTED: cluster contains multiple signals from single source. Volume ≠ confirmation.' : '',
  });
