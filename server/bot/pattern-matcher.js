...
function scorePatternMatch(cluster, pattern, clusterCategories) {
  ...
  const fpOverlap =
    pattern.common_false_positives.length > 0
      ? Math.min(
          1,
          missingPrecursors.length / pattern.common_false_positives.length,
        )
      : 0;

  const match_score = Math.min(
    1,
    precursorScore * 0.6 + geoRelevant + strengthBoost,
  );
  ...
  return {
    ...
    missing_confirmations: [
      ...
      ...(pattern.expected_confirmations || []).map(
        (c) => `Confirmation: ${c}`,
      ),
    ],
