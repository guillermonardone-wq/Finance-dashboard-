...
function validateConfidenceRange(range) {
  if (!range || typeof range !== "object")
    return { low: 0.1, high: 0.5, best: 0.3 };
  return {
...
export function validateCandidateSignal(obj) {
  ...
  if (!obj.source_refs || obj.source_refs.length === 0)
    errors.push("at least one source_ref required");
  if (!obj.categories || obj.categories.length === 0)
    errors.push("at least one category required");
  return {
    ...
    normalized: {
      ...
      categories: (obj.categories || []).filter((c) =>
        SIGNAL_CATEGORIES.includes(c),
      ),
...
export function validateSignalCluster(obj) {
  ...
  if (!obj.member_signal_ids || obj.member_signal_ids.length === 0)
    errors.push("at least one member signal required");
  return {
    valid: errors.length === 0,
    errors,
    normalized: {
      ...
      cluster_strength: CLUSTER_STRENGTH_LEVELS.includes(obj.cluster_strength)
        ? obj.cluster_strength
        : "weak",
...
export function validateMispricingAssessment(obj) {
  return {
    ...
    normalized: {
      ...
      market_reaction_state: MARKET_REACTION_STATES.includes(
        obj.market_reaction_state,
      )
        ? obj.market_reaction_state
        : "unknown",
      correlated_assets_checked: obj.correlated_assets_checked || [],
      market_signals_found: obj.market_signals_found || [],
      implied_mispricing_likelihood: clamp(
        obj.implied_mispricing_likelihood ?? 0.3,
        0,
        1,
      ),
...
export function validateCounterCase(obj) {
  ...
  if (!obj.strongest_opposing_case || obj.strongest_opposing_case.length < 20) {
    errors.push(
      "strongest_opposing_case must be substantive (>20 chars). Weak objections defeat the purpose.",
    );
  }
  return {
    valid: errors.length === 0,
    ...
    normalized: {
      ...
      pricing_already_reflects_story:
        obj.pricing_already_reflects_story || false,
...
export function validateBotRecommendation(obj) {
  ...
  if (!BOT_STATES.includes(obj.recommended_state))
    errors.push(`invalid state: ${obj.recommended_state}`);
  ...
  return {
    ...
    normalized: {
      ...
      cluster_id: obj.cluster_id || "",
      generated_at: obj.generated_at || new Date().toISOString(),
      recommended_state: BOT_STATES.includes(obj.recommended_state)
        ? obj.recommended_state
        : "LOG_ONLY",
      confidence_range: validateConfidenceRange(obj.confidence_range),
      ...
      linked_mispricing_assessment_id:
        obj.linked_mispricing_assessment_id || null,
