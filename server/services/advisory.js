...
import { ADVISORY_SYSTEM_PROMPT, PROMPT_VERSION, buildUserPrompt } from './advisory-prompt.js';
...
function parseAdvisoryResponse(content) {
  ...
  const required = [
    'evidence_strength', 'signal_independence', 'structural_logic',
    'timing_clarity', 'market_edge', 'counter_case_robustness',
    'overall_score', 'confidence_level', 'strongest_counter_case',
    'hidden_assumptions', 'key_missing_information',
    'top_supporting_signals', 'top_concerns', 'recommendation',
  ];
  ...
  if (!Array.isArray(parsed.key_missing_information)) parsed.key_missing_information = [];
  if (!Array.isArray(parsed.top_supporting_signals)) parsed.top_supporting_signals = [];
...
function computeComparison(llmScores, deterministicScores) {
  ...
  if (detComposite == null) {
    return {
      delta: null,
      summary: 'Deterministic engine has not scored this thesis yet. No comparison available.',
    };
  }
  ...
  const summaryParts = [];
  summaryParts.push(`Overall: LLM ${delta >= 0 ? '+' : ''}${delta} vs deterministic (${llmComposite} vs ${detComposite}).`);

  if (disagreements.length > 0) {
    const biggest = disagreements.sort((a, b) => Math.abs(b.delta) - Math.abs(a.delta))[0];
    summaryParts.push(`Biggest dimension gap: ${biggest.dimension} (${biggest.direction} by ${Math.abs(biggest.delta).toFixed(1)}).`);
  }

  if (severity === 'strong_disagreement') {
    summaryParts.push('Strong disagreement — review both assessments carefully before acting.');
  }
...
export async function runAdvisoryEvaluation(thesisId, options = {}) {
  ...
  if (!callLLM) throw new Error(`Unsupported LLM provider: ${provider}. Supported: ${Object.keys(PROVIDERS).join(', ')}`);
  ...
  db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, '{}', 'running')
  `).run(assessmentId, thesisId, provider, model || 'default', PROMPT_VERSION, JSON.stringify(packet));

  try {
    // Step 3: Call LLM
    console.log(`[Advisory] Calling ${provider} (model: ${model || 'default'})...`);
    const llmResult = await callLLM(ADVISORY_SYSTEM_PROMPT, userPrompt, model);

    // Step 4: Parse response
    console.log(`[Advisory] Parsing response (${llmResult.latencyMs}ms, ${llmResult.completionTokens} tokens)...`);
    const assessment = parseAdvisoryResponse(llmResult.content);

    // Step 5: Compute comparison
    const comparison = computeComparison(assessment, packet.deterministic_scores);
    ...
    `).run(
      model || 'default',
      assessment.evidence_strength, assessment.signal_independence,
      assessment.structural_logic, assessment.timing_clarity,
      assessment.market_edge, assessment.counter_case_robustness,
      assessment.overall_score, assessment.confidence_level,
      ...
      packet.deterministic_scores.composite_score, comparison.delta,
      comparison.summary,
      JSON.stringify(llmResult.raw),
      llmResult.promptTokens, llmResult.completionTokens, llmResult.latencyMs,
      assessmentId
    );

    console.log(`[Advisory] Assessment ${assessmentId} completed. LLM: ${assessment.overall_score}/100, Det: ${packet.deterministic_scores.composite_score ?? '—'}, Delta: ${comparison.delta ?? '—'}`);

    // Return the full record
    const stored = db.prepare('SELECT * FROM llm_thesis_assessments WHERE id = ?').get(assessmentId);
    ...
    console.error(`[Advisory] Assessment ${assessmentId} FAILED: ${err.message}`);
    throw err;
  }
}
...
function parseAssessmentRow(row) {
  ...
  const jsonFields = [
    'hidden_assumptions', 'key_missing_information',
    'top_supporting_signals', 'top_concerns',
    'thesis_packet_json', 'raw_response_json',
  ];
  const parsed = { ...row };
