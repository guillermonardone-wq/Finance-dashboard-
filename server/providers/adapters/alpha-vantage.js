...
export class AlphaVantageProvider extends BaseProvider {
  ...
  async initialize() {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}?function=GLOBAL_QUOTE&symbol=SPY&apikey=${this.apiKey}`,
      );
  ...
  async getPrice(symbol) {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}?function=GLOBAL_QUOTE&symbol=${encodeURIComponent(symbol)}&apikey=${this.apiKey}`,
      );
      const data = await res.json();
  ...
  async getCandles(symbol, interval = "1d", from, to) {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}?function=${fn}&symbol=${encodeURIComponent(symbol)}&outputsize=compact&apikey=${this.apiKey}`,
      );
      const data = await res.json();
  ...
  async getMacroSeries(seriesId) {
    ...
    try {
      const res = await fetch(
        `${BASE_URL}?function=${series.fn}&apikey=${this.apiKey}`,
      );
      const data = await res.json();
  ...
  async getNews(query) {
    ...
    try {
      ...
      const res = await fetch(
        `${BASE_URL}?function=NEWS_SENTIMENT${tickers}&apikey=${this.apiKey}`,
      );
      const data = await res.json();
