...
router.post('/providers', (req, res) => {
  ...
  if (!name || !provider_key) {
    return res.status(400).json({ error: 'name and provider_key are required' });
  }
...
router.post('/events', (req, res) => {
  ...
  if (!provider_id || !title) {
    return res.status(400).json({ error: 'provider_id and title are required' });
  }
...
router.post('/links', (req, res) => {
  ...
  if (!thesis_id || !prediction_market_event_id) {
    return res.status(400).json({ error: 'thesis_id and prediction_market_event_id are required' });
  }
