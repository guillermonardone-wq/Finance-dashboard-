...
import { runAdvisoryEvaluation, getLatestAssessment, getAssessmentHistory } from '../services/advisory.js';
...
router.post('/thesis/:thesisId/evaluate', async (req, res) => {
  ...
  console.log(`[Advisory] POST /thesis/${thesisId}/evaluate — provider: ${provider || 'default'}`);

  try {
    const assessment = await runAdvisoryEvaluation(thesisId, { provider, model });
    res.json(assessment);
  } catch (err) {
