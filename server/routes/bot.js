...
router.post('/run', (req, res) => {
  try {
    ...
    if (!inputs || !Array.isArray(inputs)) {
      return res.status(400).json({ error: 'inputs must be an array of { type, data } objects' });
    }
...
router.get('/runs', (req, res) => {
  try {
    ...
    res.json(runs.map(r => {
      try { r.summary = JSON.parse(r.summary); } catch {}
      return r;
    }));
  } catch (err) {
    res.status(500).json({ error: err.message });
...
router.get('/runs/:id', (req, res) => {
  try {
    const db = getDb();
    const run = db.prepare('SELECT * FROM bot_pipeline_runs WHERE id = ?').get(req.params.id);
    if (!run) return res.status(404).json({ error: 'Run not found' });
    try { run.summary = JSON.parse(run.summary); } catch {}
    try { run.full_result = JSON.parse(run.full_result); } catch {}
    res.json(run);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get('/recommendations', (req, res) => {
  try {
    ...
    const params = [];
    if (state) { sql += ' AND recommended_state = ?'; params.push(state); }
