...
router.get("/candles/:symbol", async (req, res) => {
  try {
    ...
    const result = await ingestion.fetchCandles(
      req.params.symbol,
      interval || "1d",
      from,
      to,
...
router.get("/fred/yield-curve", async (req, res) => {
  try {
    ...
    if (!provider?.enabled) {
      return res.json({
        success: false,
        error: "FRED provider not configured. Add FRED_API_KEY to .env",
        data: [],
      });
    }
    ...
    res.json({
      ...
      data,
      provider: "fred",
      source_attribution: "FRED (Federal Reserve) — Yield Curve",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/fred/series", (req, res) => {
  ...
  if (!provider) {
    return res.json({
      success: false,
      error: "FRED provider not registered",
      data: [],
    });
  }
...
router.get("/fred/series/:seriesId", async (req, res) => {
  try {
    ...
    res.json({
      ...
      data,
      provider: "fred",
      source_attribution: `FRED — ${req.params.seriesId}`,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/fred/calendar", async (req, res) => {
  try {
    ...
    res.json({
      ...
      data,
      provider: "fred",
      source_attribution: "FRED — Release Calendar",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.post("/worldbank/search", async (req, res) => {
  try {
    ...
    if (!provider?.enabled) {
      return res.json({
        success: false,
        error: "World Bank provider unavailable",
        data: [],
      });
    }
    ...
    res.json({
      ...
      data,
      provider: "worldbank",
      source_attribution: "World Bank Data360",
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/worldbank/indicators", (req, res) => {
  ...
  if (!provider) {
    return res.json({
      success: false,
      error: "World Bank provider not registered",
      data: [],
    });
  }
  res.json({
...
router.get("/worldbank/data/:indicator", async (req, res) => {
  try {
    ...
    res.json({
      ...
      data,
      provider: "worldbank",
      source_attribution: `World Bank Data360 — ${req.params.indicator}`,
    });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
...
router.get("/fx/rates", async (req, res) => {
  try {
    ...
    if (!provider?.enabled) {
      return res.json({
        success: false,
        error: "Finnhub provider not configured. Add FINNHUB_API_KEY to .env",
        data: null,
      });
    }
...
router.get("/fx/quote/:pair", async (req, res) => {
  try {
    ...
    if (!provider?.enabled) {
      return res.json({
        success: false,
        error: "Finnhub provider not configured",
        data: null,
      });
    }
