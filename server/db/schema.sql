...
CREATE TABLE IF NOT EXISTS theses (
  ...
  -- Scoring — Evidence Layer (computed by engine)
  ...
  score_evidence_layer REAL,              -- aggregated evidence layer score (0-10)
  ...
  score_structure_layer REAL,             -- aggregated structure layer score (0-10)
  ...
  score_market_edge_layer REAL,           -- aggregated market edge layer score (0-10)

  -- Composite and penalties
  composite_score REAL,
  penalty_total REAL,                     -- total penalty points deducted
  penalty_details TEXT,                   -- JSON: array of { id, label, value, reason }

  -- Confidence (separate from score)
  confidence_level REAL,                  -- 0-1: how confident we are in the score
  confidence_factors TEXT,                -- JSON: breakdown of confidence factors

  -- Calibration (for comparing predicted vs actual)
  score_at_creation REAL,                 -- composite score when thesis was first scored
  score_at_approval REAL,                 -- composite score when thesis was approved
  classification_at_creation TEXT,        -- classification at first scoring
  final_outcome TEXT,                     -- JSON: { result, actual_return, timeline_accuracy, ... }

  -- Legacy compatibility (deprecated, kept for migration)
...
-- ============================================================
-- LLM ADVISORY ASSESSMENTS: Parallel AI evaluation of theses
-- ============================================================
-- These are ADVISORY ONLY. They cannot override deterministic
-- scoring, bypass gates, or set trade state. They provide a
-- second opinion for the human operator to consider.
-- ============================================================
CREATE TABLE IF NOT EXISTS llm_thesis_assessments (
  id TEXT PRIMARY KEY,
  thesis_id TEXT NOT NULL REFERENCES theses(id) ON DELETE CASCADE,
  created_at TEXT NOT NULL DEFAULT (datetime('now')),

  -- Model identity
  model_provider TEXT NOT NULL,          -- 'anthropic', 'openai', etc.
  model_name TEXT NOT NULL,              -- 'claude-sonnet-4-6', 'gpt-4o', etc.
  prompt_version TEXT NOT NULL,          -- semver for prompt tracking

  -- Dimension scores (0-10, mirroring deterministic engine)
  score_evidence_strength REAL,
  score_signal_independence REAL,
  score_structural_logic REAL,
  score_timing_clarity REAL,
  score_market_edge REAL,
  score_counter_case_robustness REAL,

  -- Aggregate
  overall_score REAL,                    -- 0-100
  confidence_level REAL,                 -- 0-1: LLM's confidence in its own assessment

  -- Qualitative outputs
  strongest_counter_case TEXT,
  hidden_assumptions TEXT,               -- JSON array of strings
  key_missing_information TEXT,          -- JSON array of strings
  top_supporting_signals TEXT,           -- JSON array of strings
  top_concerns TEXT,                     -- JSON array of strings
  recommendation TEXT,                   -- structured advisory text

  -- Comparison snapshot (captured at assessment time)
  deterministic_score REAL,              -- composite_score at time of LLM run
  score_delta REAL,                      -- LLM overall - deterministic
  disagreement_summary TEXT,             -- human-readable delta explanation

  -- Raw data (for auditing and prompt iteration)
  thesis_packet_json TEXT NOT NULL,      -- full context sent to LLM
  raw_response_json TEXT NOT NULL,       -- full LLM response
...
CREATE INDEX IF NOT EXISTS idx_llm_assessments_thesis ON llm_thesis_assessments(thesis_id);
CREATE INDEX IF NOT EXISTS idx_llm_assessments_created ON llm_thesis_assessments(created_at);
