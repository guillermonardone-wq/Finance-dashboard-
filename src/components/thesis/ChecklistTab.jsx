...
export default function ChecklistTab({ thesis }) {
  ...
  return (
    <div className="space-y-6">
      {/* Summary bar */}
      <div
        className={`p-4 rounded border ${result.passed ? "border-emerald-400/30 bg-emerald-400/5" : "border-red-400/30 bg-red-400/5"}`}
      >
        <div className="flex items-center justify-between mb-2">
          <span
            className={`font-bold text-sm ${result.passed ? "text-emerald-400" : "text-red-400"}`}
          >
            {result.summary}
          </span>
      ...
      {result.warnings.length > 0 && (
        <div className="p-3 rounded border border-amber-400/20 bg-amber-400/5">
          ...
          {result.warnings.map((w) => (
            <p key={w.id} className="text-xs text-amber-300 mb-1">
              {w.message}
            </p>
          ))}
        </div>
