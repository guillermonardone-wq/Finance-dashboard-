...
export const PRE_TRADE_CHECKLIST = [
  ...
  },
  {
    ...
    question:
      "What specific, observable condition would INVALIDATE this thesis?",
    type: "text",
    required: true,
    minLength: 20,
  ...
  },
  {
    id: "timing_bounded",
    question:
      "Is the thesis timeline bounded with a specific review/exit date?",
    type: "boolean",
    required: true,
  ...
  },
  {
    id: "real_mispricing",
    question:
      "Is there MEASURABLE market mispricing, or is this just a dramatic narrative?",
    type: "select",
    options: [
      {
        value: "measurable",
        label: "Yes — I can point to specific mispricing",
      },
      {
        value: "probable",
        label: "Probable — indirect evidence of mispricing",
      },
      {
        value: "narrative_only",
        label: "Honestly, this is more narrative than evidence",
      },
    ],
    required: true,
    failValues: ["narrative_only"],
    failMessage:
      'STOP. "Dramatic" ≠ "mispriced." Interesting stories lose money all the time.',
    category: "asymmetry",
  },
  {
    id: "clean_vehicle",
    ...
    required: true,
    failMessage:
      "No clean vehicle = no trade. Forcing a bad vehicle ruins asymmetry.",
    category: "execution",
  },
  {
    id: "risk_capped",
    question:
      "Is your maximum loss pre-defined and acceptable BEFORE the trade?",
    type: "boolean",
    required: true,
  ...
  },
  {
    ...
    warningThreshold: 3,
    warningMessage:
      "Fewer than 3 independent signals. Thin evidence = thin conviction.",
    category: "evidence",
  },
  {
    id: "data_fresh",
    question:
      "Is your supporting data fresh (< 24 hours for market data, < 48 hours for macro)?",
    type: "boolean",
    required: true,
    failMessage:
      "Stale data may mean the market has already moved. Refresh before acting.",
    category: "evidence",
  },
  {
    id: "process_not_excitement",
    question:
      "Am I acting from disciplined process, or because this feels exciting?",
    type: "select",
    options: [
      ...
      {
        value: "excitement",
        label: "Honestly, this is exciting and I want to act",
      },
    ],
    required: true,
    ...
    warnMessage:
      "Notice the excitement. Reduce position size by 50% as a guardrail.",
    category: "behavioral",
  },
  {
    ...
    required: true,
    failMessage:
      "OWNERSHIP BIAS. You are attached to THIS idea, not its quality. Step back.",
    category: "behavioral",
  },
  {
    id: "early_vs_right",
    ...
    minLength: 20,
    failMessage:
      '"Being early" is the most common excuse for being wrong. Define the difference.',
    category: "disconfirmation",
  },
  {
    id: "emotional_state",
    ...
    warnValues: ["anxious"],
    failMessage:
      "Your emotional state DISQUALIFIES action right now. Quarantine and revisit in 12+ hours.",
    warnMessage:
      "Anxiety clouds judgment. Reduce position size and set tighter stops.",
    category: "behavioral",
  },
];
  ...
  for (const item of PRE_TRADE_CHECKLIST) {
    ...
    if (answer === undefined || answer === null || answer === "") {
      ...
      if (
        ...
        answer.length >= (item.minLength || 1)
      ) {
        status = "passed";
        passedCount++;
      } else {
...
export const POSITION_SIZING_RULES = {
  ...
  FULLY_QUALIFIED: {
    maxRiskPercent: 5,
    description: "Full evidence, full process. Up to 5% risk.",
  },
  SMALL_POSITION: {
    maxRiskPercent: 2,
    description: "Strong but not complete. Cap at 2% risk.",
  },
  PAPER_TRADE: {
    maxRiskPercent: 0,
    description: "Paper trade only. No real capital.",
  },
  DEVELOP: {
    maxRiskPercent: 0,
    description: "Still developing. No capital at risk.",
  },
  WATCH: { maxRiskPercent: 0, description: "Watch only. Not actionable." },
  IGNORE: { maxRiskPercent: 0, description: "Noise. Move on." },
  QUARANTINED: {
    maxRiskPercent: 0,
    description: "Quarantined. Absolutely no action.",
  },
};
