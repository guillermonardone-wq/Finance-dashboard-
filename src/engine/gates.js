...
export const GATE_DEFINITIONS = [
  {
    ...
    description:
      "A specific, observable condition that would prove the thesis wrong.",
    ...
    check: (thesis) => {
      ...
      const hasInv =
        ...
        (thesis.invalidation_condition &&
          thesis.invalidation_condition.length > 10);
      return {
        passed: hasInv,
        reason: hasInv
  ...
  },
  {
    ...
    description:
      "When should this play out? Unbounded theses are undisciplined.",
    severity: "hard",
    check: (thesis) => {
  ...
  },
  {
    ...
    check: (thesis, executionPlan) => {
      const has =
        executionPlan &&
        (executionPlan.max_risk_dollars || executionPlan.max_risk_percent);
      return {
        passed: !!has,
        reason: has
  ...
  },
  {
    ...
    check: (thesis, executionPlan) => {
      const has =
        executionPlan &&
        executionPlan.expression_vehicle &&
        executionPlan.expression_vehicle.length > 3;
      return {
        passed: !!has,
        reason: has
  ...
  },
  {
    ...
    check: (thesis, _, signals) => {
      const linked = (signals || []).filter(
        (s) => s.thesis_id === thesis.id && s.status === "linked",
      );
  ...
  },
  {
    ...
    check: (thesis, executionPlan, _, checklistAnswers) => {
      if (!checklistAnswers)
        return {
          passed: false,
          reason: "Emotional state not assessed. Complete the checklist.",
        };
    ...
    check: (thesis) => {
      if (thesis.quarantine_until) {
        ...
        if (now < until) {
          ...
          return {
            passed: false,
            reason: `COOLING OFF: ${hoursLeft} hours remaining. Forced delay to prevent impulsive action.`,
          };
        }
      ...
      const ageHours =
        (Date.now() - new Date(thesis.created_at).getTime()) / (1000 * 3600);
  ...
  },
  {
    ...
    check: (thesis) => {
      const spread =
        (thesis.probability_high || 0) - (thesis.probability_low || 0);
  ...
  },
  {
    ...
    check: (thesis, _, __, checklistAnswers) => {
      if (!checklistAnswers)
        return { passed: false, reason: "Not yet assessed." };
