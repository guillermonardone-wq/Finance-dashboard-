...
test('initDb creates tables without error', () => {
  ...
  assert.ok(tables.includes('market_observations'), 'market_observations table exists');
...
test('seed creates market observations', () => {
  const db = getDb();
  const obsCount = db.prepare('SELECT COUNT(*) as count FROM market_observations').get().count;
  assert.ok(obsCount >= 5, `Expected >= 5 observations, got ${obsCount}`);
});

test('seeded theses have correct status and classification', () => {
  const db = getDb();
  const active = db.prepare("SELECT * FROM theses WHERE status = 'active'").all();
  assert.ok(active.length >= 1, 'At least one active thesis');
  for (const t of active) {
    assert.ok(['WATCH', 'DEVELOP', 'PAPER_TRADE', 'SMALL_POSITION', 'FULLY_QUALIFIED', 'IGNORE'].includes(t.classification),
...
test('creating a draft thesis requires only title and statement', () => {
  ...
  db.prepare(`INSERT INTO theses (
    ...
    id, now, now,
    'Test Quick Capture', 'This is a test thesis statement',
    '[]', '[]', '{}', 0.2, 0.6, 0.4, '{}', '[]', '[]',
    '[]', '', '', 'draft', 'WATCH', '[]'
  );
...
await asyncTest('registry returns graceful error when no provider enabled', async () => {
  const { registry } = await import('../server/providers/registry.js');
...
await asyncTest('registry returns graceful error for macro series without provider', async () => {
  const { registry } = await import('../server/providers/registry.js');
  const result = await registry.getMacroSeries('US_CPI');
  assert.equal(result.success, false, 'Should fail gracefully');
  assert.ok(result.error.includes('No enabled provider'), 'Error mentions no provider');
});
...
test('World Bank provider has correct capabilities', async () => {
  const { WorldBankProvider } = await import('../server/providers/adapters/worldbank.js');
...
test('World Bank getAvailableIndicators returns curated list', async () => {
  ...
  assert.ok(indicators.length >= 5, `Expected >= 5 indicators, got ${indicators.length}`);
...
test('Finnhub provider has correct capabilities', async () => {
  const { FinnhubProvider } = await import('../server/providers/adapters/finnhub.js');
...
test('FX pairs list covers major USD crosses', () => {
  ...
  assert.ok(FX_PAIRS.every(p => p.includes('USD')), 'All pairs include USD');
});
...
await asyncTest('bot full scan runs without crashing', async () => {
  ...
  assert.ok(['complete', 'empty', 'error'].includes(result.status), `Status is valid: ${result.status}`);
