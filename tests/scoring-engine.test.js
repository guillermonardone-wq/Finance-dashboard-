...
describe("estimateConfidence", () => {
  ...
  it("increases confidence with high completeness", () => {
    const low = estimateConfidence(
      { completeness: 20, missing: ALL_FACTOR_KEYS.slice(0, 12) },
      [],
      [],
      null,
    );
    const high = estimateConfidence(
  ...
  it("reduces confidence for missing factors", () => {
    const complete = estimateConfidence(
      { completeness: 100, missing: [] },
      [],
      [],
      null,
    );
    const incomplete = estimateConfidence(
      { completeness: 30, missing: ALL_FACTOR_KEYS.slice(0, 10) },
      [],
      [],
      null,
    );
...
describe("computeCompositeScore", () => {
  ...
  it("subtracts penalties from raw composite", () => {
    ...
    const withPenalty = computeCompositeScore(scores, {
      items: [{ active: true, value: 10 }],
    });
...
describe("computePenalties", () => {
  ...
  it("does not penalize when invalidation exists", () => {
    const result = computePenalties({}, [], {
      invalidating_indicators: [{ indicator: "CPI" }],
    });
    const p = result.items.find((i) => i.id === "missing_invalidation");
  ...
  it("detects single source signals", () => {
    ...
    const result = computePenalties({}, signals, {
  ...
  it("detects stale data", () => {
    const result = computePenalties({ evidence_freshness: 2 }, [], {
      invalidating_indicators: ["x"],
    });
    ...
    const result = computePenalties(
      {
        ...
        counter_case_robustness: 0,
        internal_consistency: 10,
        market_awareness: 10,
        asset_reaction_gaps: 0,
      },
...
describe("scorePredictionMarketDivergence", () => {
  ...
  it("returns null for insufficient contracts", () => {
    const result = scorePredictionMarketDivergence({
      scoring_helpers: {
        ...
        prediction_market_divergence: 0.5,
        prediction_market_confidence: 0.8,
      },
    });
  ...
  it("returns null for low confidence", () => {
    const result = scorePredictionMarketDivergence({
      scoring_helpers: {
        qualifying_contract_count: 5,
        prediction_market_divergence: 0.5,
        prediction_market_confidence: 0.3,
      },
    });
    expect(result.score).toBeNull();
  });
  ...
  it("caps at 8/10", () => {
    const result = scorePredictionMarketDivergence({
      scoring_helpers: {
        qualifying_contract_count: 10,
        prediction_market_divergence: 100,
        prediction_market_confidence: 0.95,
      },
    });
...
describe("runGates", () => {
  it("passes all gates with complete thesis + execution plan", () => {
    ...
    const signals = makeSignals(5).map((s) => ({
      ...s,
      thesis_id: thesis.id,
      status: "linked",
    }));
  ...
  it("blocks on dangerous emotional state", () => {
    const result = runGates(makeThesis(), null, [], {
  ...
  it("soft-fails on few signals", () => {
    const thesis = makeThesis();
    const signals = [
      { thesis_id: thesis.id, status: "linked", source_type: "a" },
    ];
    const result = runGates(thesis, null, signals);
    const fail = result.softFails.find(
      (f) => f.id === "min_confirming_signals",
    );
    expect(fail).toBeDefined();
  });
...
describe("classifyThesis — forced downgrades", () => {
  ...
  it("caps at WATCH when signal_independence < 3", () => {
    const r = classifyThesis(
      90,
      makeScores({ signal_independence: 2 }),
      passingGates,
      thesis,
      goodLayers,
    );
  ...
  it("caps at DEVELOP when evidence_freshness < 3", () => {
    const r = classifyThesis(
      90,
      makeScores({ evidence_freshness: 2 }),
      passingGates,
      thesis,
      goodLayers,
    );
  ...
  it("caps at WATCH when causal_chain_clarity < 3", () => {
    const r = classifyThesis(
      90,
      makeScores({ causal_chain_clarity: 2 }),
      passingGates,
      thesis,
      goodLayers,
    );
  ...
  it("caps at DEVELOP when counter_case_robustness < 4", () => {
    const r = classifyThesis(
      90,
      makeScores({ counter_case_robustness: 3 }),
      passingGates,
      thesis,
      goodLayers,
    );
...
describe("classifyThesis — gate-driven downgrades", () => {
  ...
  it("caps at DEVELOP when hard gates fail", () => {
    const failedGates = makeGateResult({
      passed: false,
      hardFails: [
        { id: "invalidation_defined", name: "Invalidation", severity: "hard" },
      ],
    });
  ...
  it("downgrades by 2 levels for 4 soft failures", () => {
    const softGates = makeGateResult({
      softFails: Array.from({ length: 4 }, (_, i) => ({
        id: `s${i}`,
        name: `Soft ${i}`,
        severity: "soft",
      })),
...
describe("classifyThesis — assumption overload", () => {
  ...
  it("caps at WATCH for > 7 assumptions", () => {
    const thesis = makeThesis({
      key_assumptions: Array.from({ length: 8 }, (_, i) => `A${i}`),
    });
    const r = classifyThesis(
      90,
      makeScores(),
  ...
  it("caps at DEVELOP for 6-7 assumptions", () => {
    const thesis = makeThesis({
      key_assumptions: Array.from({ length: 6 }, (_, i) => `A${i}`),
    });
    const r = classifyThesis(
      90,
      makeScores(),
...
describe("runChecklist", () => {
  ...
  it("passes with complete correct answers", () => {
    const answers = {
      ...
      invalidation_defined:
        "If CPI rises above 4% for two consecutive quarters",
      timing_bounded: true,
      ...
      would_take_if_not_mine: true,
      early_vs_right:
        "If employment data reverses for 3 months without rate action, I am wrong not early",
      emotional_state: "calm",
    };
  ...
  it("blocks on excitement-driven action", () => {
    ...
    const result = runChecklist(answers);
    expect(result.passed).toBe(false);
    expect(result.blocks.some((b) => b.id === "process_not_excitement")).toBe(
      true,
    );
  });
  ...
  it("warns on few independent signals", () => {
    const answers = {
      ...
      would_take_if_not_mine: true,
      early_vs_right:
        "If employment reverses for 3 months without action, I am wrong",
      emotional_state: "calm",
    };
    const result = runChecklist(answers);
    expect(result.warnings.some((w) => w.id === "independent_signals")).toBe(
      true,
    );
...
describe("getMaxPositionSize", () => {
  ...
  it("returns description string for every classification", () => {
    for (const c of [
      "FULLY_QUALIFIED",
      "SMALL_POSITION",
      "PAPER_TRADE",
...
// ============================================================
// classifyThesis — Additional boundary tests
// ============================================================

describe("classifyThesis — exact boundary transitions", () => {
  const passingGates = makeGateResult();
  const thesis = makeThesis();
  const goodLayers = makeLayerScores();

  it("classifies PAPER_TRADE at score 74 (top of tier)", () => {
    const r = classifyThesis(
      74,
      makeScores(),
      passingGates,
  ...
  it("transitions PAPER_TRADE→SMALL_POSITION at 74→75", () => {
    const at74 = classifyThesis(
      74,
      makeScores(),
      passingGates,
      thesis,
    ...
    expect(at74.classification).toBe("PAPER_TRADE");
    expect(at75.classification).toBe("SMALL_POSITION");
  });

  it("transitions SMALL_POSITION→FULLY_QUALIFIED at 84→85", () => {
    const at84 = classifyThesis(
      84,
      makeScores(),
      passingGates,
      thesis,
    ...
    expect(at84.classification).toBe("SMALL_POSITION");
    expect(at85.classification).toBe("FULLY_QUALIFIED");
  });

  it("classifies score 0 as IGNORE", () => {
    const r = classifyThesis(
      0,
      makeScores(),
      passingGates,
...
// ============================================================
// classifyThesis — Output shape validation
// ============================================================

describe("classifyThesis — output shape", () => {
  it("returns all required fields", () => {
    ...
    expect(r).toHaveProperty("scoreClassification");
    expect(r).toHaveProperty("composite");
    expect(r).toHaveProperty("reasons");
    expect(r).toHaveProperty("downgrades");
    expect(r).toHaveProperty("overrideActive");
    expect(r).toHaveProperty("meta");
    expect(Array.isArray(r.reasons)).toBe(true);
    expect(Array.isArray(r.downgrades)).toBe(true);
    expect(typeof r.overrideActive).toBe("boolean");
  });
});

// ============================================================
// runChecklist — Output shape validation
// ============================================================

describe("runChecklist — output shape", () => {
  it("returns all required fields on empty input", () => {
    const result = runChecklist({});
  ...
  it("items have correct structure", () => {
    const result = runChecklist({ opposite_case: true });
    for (const item of result.items) {
  ...
  it("total matches PRE_TRADE_CHECKLIST length (12)", () => {
    const result = runChecklist({});
    expect(result.total).toBe(12);
  });

  it("blocks on all four dangerous emotional states", () => {
    for (const state of ["excited", "revenge", "fomo", "bored"]) {
      const result = runChecklist({ emotional_state: state });
      expect(
        result.blocks.some((b) => b.id === "emotional_state"),
        `expected block for ${state}`,
      ).toBe(true);
    }
  });

  it("warns on anxious emotional state without blocking", () => {
    const result = runChecklist({ emotional_state: "anxious" });
    expect(
      result.warnings.some((w) => w.id === "emotional_state"),
    ).toBe(true);
    expect(
      result.blocks.some((b) => b.id === "emotional_state"),
    ).toBe(false);
  ...
  it("warns on mostly_process without blocking", () => {
    const result = runChecklist({
      process_not_excitement: "mostly_process",
    });
    expect(
      result.warnings.some((w) => w.id === "process_not_excitement"),
    ).toBe(true);
    expect(
      result.blocks.some((b) => b.id === "process_not_excitement"),
    ).toBe(false);
  });
});

// ============================================================
// runGates — Output shape validation
// ============================================================

describe("runGates — output shape", () => {
  it("returns all required fields", () => {
    const result = runGates(makeThesis());
    expect(result).toHaveProperty("passed");
    expect(result).toHaveProperty("total");
    ...
    expect(result).toHaveProperty("softFails");
    expect(result).toHaveProperty("results");
    expect(typeof result.passed).toBe("boolean");
    expect(typeof result.total).toBe("number");
    expect(Array.isArray(result.hardFails)).toBe(true);
    expect(Array.isArray(result.softFails)).toBe(true);
    expect(Array.isArray(result.results)).toBe(true);
  });

  it("each result has id, name, severity, passed", () => {
    const result = runGates(makeThesis());
...
// ============================================================
// computeCompositeScore — Additional edge cases
// ============================================================

describe("computeCompositeScore — edge cases", () => {
  it("handles null/undefined scores gracefully", () => {
    const result = computeCompositeScore({
      signal_quality: 7,
      signal_independence: null,
      evidence_freshness: undefined,
    });
    expect(result.composite).toBeGreaterThanOrEqual(0);
    expect(result.missing).toContain("signal_independence");
    expect(result.missing).toContain("evidence_freshness");
  });

  it("returns layers object with evidence, structure, market_edge", () => {
    const result = computeCompositeScore(makeScores());
    expect(result.layers).toHaveProperty("evidence");
    expect(result.layers).toHaveProperty("structure");
    expect(result.layers).toHaveProperty("market_edge");
    expect(typeof result.layers.evidence.score).toBe("number");
    expect(typeof result.layers.structure.score).toBe("number");
    expect(typeof result.layers.market_edge.score).toBe("number");
  });
});
