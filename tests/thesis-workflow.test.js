...
beforeAll(() => {
  initDb();
  // Run migrations
  import('../server/db/migrate-scoring-v2.js').then(m => m.migrateScoringV2());
  import('../server/db/migrate-source-types.js').then(m => m.migrateSourceTypes());
});
...
describe('Thesis CRUD', () => {
  ...
  it('creates a draft thesis with minimal fields', () => {
    const created = thesisRepo.create(thesisId, {
      ...
      thesis_statement: 'The Fed will cut rates by September due to slowing employment.',
      status: 'draft',
    });
  ...
  it('updates thesis fields', () => {
    ...
    expect(updated.causal_chain).toEqual(['Employment slows', 'CPI drops', 'Fed signals pivot']);
  });
...
describe('Scoring Engine', () => {
  it('computes composite score from factor scores', async () => {
    const { computeCompositeScore, computePenalties } = await import('../src/engine/scoring.js');
    ...
    const penalties = computePenalties(scores, [], { invalidating_indicators: ['test'] });
  ...
  it('applies penalties for missing invalidation', async () => {
    ...
    const missing = penalties.items.find(p => p.id === 'missing_invalidation');
  ...
  it('detects single-source penalty', async () => {
    ...
    const penalties = computePenalties({}, signals, { invalidating_indicators: ['x'] });
...
describe('Classification', () => {
  it('classifies based on composite score', async () => {
    ...
    const result = classifyThesis(
      ...
      { status: 'active' },
      { evidence: { score: 8 }, structure: { score: 8 }, market_edge: { score: 7 } }
    );
